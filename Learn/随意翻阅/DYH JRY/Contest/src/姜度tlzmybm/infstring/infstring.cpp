...
ENS int main()
{
	freopen("infstring.in","r",stdin);freopen("infstring.out","w",stdout);int i,x,y,n,test,now,e,tp,q,last=1,cnt=1;char c;
	...
	for (last=len-(len%n)-n;test--;)
		if (read(x),(--x)<=len) print(MAPLE[x]);
		else tp=last+(x%n),print(MAPLE[tp]+(MAPLE[tp]-MAPLE[tp-n])*((x-tp)/n));
